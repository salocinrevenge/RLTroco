...
def main():
    ...
    v_values = calculate_v_values(train_env, tf_agent.policy)
    plot_v_values(v_values, s)
...
def calculate_v_values(env, policy):
    num_states = env.observation_spec().maximum + 1
    v_values = np.zeros(num_states)
    for state in range(num_states):
        time_step = trajectories.time_step.TimeStep(
            step_type=np.array(0, dtype=np.int32),
            reward=np.array(0.0, dtype=np.float32),
        ...
        q_values = np.array(scores)[:, 0]
        v_values[state] = q_values[action_step.action.numpy()]
    return v_values

def plot_v_values(v_values, s):
    v_values = v_values.reshape((8, 8))  # Reshape to match the shape of the environment
    pl.clf()
    pl.imshow(v_values, cmap='hot', interpolation='nearest')
    pl.colorbar()
    pl.savefig(f"v_values{s}.png")
    return v_values
